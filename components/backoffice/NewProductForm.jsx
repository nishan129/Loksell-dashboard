...
export default function NewProductsForm({ categories, wholesaller, updateData = {} }) {
    ...
    const productImagesUrl = updateData.productImages || [];
    const initialTags = updateData.tags || [];
    const id = updateData.id || "";

    const [productImages, setProductImages] = useState(productImagesUrl);
    ...
    const handleSubmitForm = async (data) => {
        ...
        if (id) {
            productData.id = id;
            await makePutRequest(setLoading, `api/products/${id}`, productData, "Products", handleRedirect, reset);
        } else {
            await makePostRequest(setLoading, 'api/products', productData, "Products", reset, handleRedirect);
    ...
    return (
        <form onSubmit={handleSubmit(handleSubmitForm)}
            ...
            <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
                ...
                <TextInput
                    label="Unit of Measurement (e.g., Kilograms, Grams)"
                    name="unit"
                    register={register}
                    errors={errors}
                    className='w-full' />
